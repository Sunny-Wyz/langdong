...
Q - 需求预测结果页面（AiForecastResult.vue）无数据，之前都有数据 - 根因是最近前端代码修改时在模板（`<template>`）中使用了 `getAlgoDisplayName(scope.row.algoType)`，但在 `<script>` 的 `methods` 中漏写了 `getAlgoDisplayName` 方法的定义。由于该方法未定义，Vue 在试图渲染表格行时触发了运行时 `ReferenceError` 错误，导致表格崩溃并最终显示为空白无数据。解决方案：在 `AiForecastResult.vue` 文件的 `methods` 中补齐 `getAlgoDisplayName` 方法，使其对不同算法类型（如 RF、SBA、FALLBACK 等）分别映射为友好显示的中文名称（如“随机森林 (RF)”、“SBA 算法”、“两阶段概率预测模型”等）。修复后渲染错误消除，页面数据可正常呈现。

Q - 管理层看板（Dashboard.vue）设备可用率卡片跳过空了一行 - 根因是在最近为看板引入“设备可用率”卡片后，看板的 KPI 卡片增加到了 5 张；但前端仍然采用 Element UI 默认 of 24 栅格布局（每个卡片配置为 `:lg="6" :xl="6"`，即大屏下每行固定容纳 4 个卡片，6*4=24）。由于 24 无法被 5 整除，导致第五张卡片（设备可用率）被强制折行，在大屏上单独占据第二行左侧并留下右侧大面积空白。解决方案：放弃传统的 `el-row` / `el-col` 24 栅格布局，针对 KPI 区域引入现代化自定义的 CSS Grid 响应式网格布局（`.kpi-grid`），配置 `grid-template-columns: repeat(5, minmax(0, 1fr))` 完美实现大屏下 5 张卡片均分单行且完美对齐；同时添加 `@media` 响应式查询在中/小屏幕下自动平滑降级为 3 列、2 列和 1 列排版，消除了“换行孤立占位”的突兀排版，视觉呈现更合理 premium。

Q - 管理层看板界面数据全空（显示 0 或 N/A） - 根因是系统当前的实际时间为 2026 年 6 月，但数据库中已有的测试业务数据（工单、领用、采购等）日期止于 2026 年 2~4 月。看板首屏默认以当前自然月（2026-06）以及上月（2026-05）进行聚合查询并计算环比，无对应月份记录导致各项 KPI 卡片和图表归零。解决方案：编写并执行了 SQL 脚本生成了 2026-05 和 2026-06 完整的、涵盖工单、领用明细、采购订单、可用率等的测试数据。执行后，今日待办、库存周转、采购、维修、可用率、维修费构成、消耗趋势等模块均自动加载并呈现出了生动的、带环比波动的真实模拟运营指标，界面不再空白。




Q - XGBoost4J 2.1.0 在 Apple Silicon (Mac arm64) 上加载报错 UnsatisfiedLinkError: Library not loaded: @rpath/libomp.dylib - 根因是 xgboost4j jar 内已含 lib/macos/aarch64/libxgboost4j.dylib（原生库本身支持 arm64），但该 dylib 动态链接了 OpenMP 运行时 libomp.dylib，而系统未安装。dlopen 在 /opt/homebrew/opt/libomp/lib 等路径找不到 libomp 导致加载失败。解决方案：执行 `brew install libomp`（装到 /opt/homebrew/opt/libomp/lib/libomp.dylib，正是 dylib 默认搜索路径）。装完后 XGBoost 训练/预测、以及 reg:quantileerror + quantile_alpha 原生分位数 objective 均在 aarch64 正常运行，无需退化为自定义 pinball loss。注意：部署/运行 SpringBoot 的机器都必须预装 libomp。

Q - M5 阶段运行单元测试时找不到 mvn 命令，且使用 -pl backend 时提示 Could not find the selected project in the reactor - 根因一是 mvn 并未在全局 PATH 中（Maven 安装于自定义路径 `/Users/weiyaozhou/IdeaProjects/apache-maven-3.8.8-bin`）；根因二是 Maven 命令在根目录下直接执行时无法正确识别子项目。解决方案：1）将工作目录切换至 `backend` 子目录下；2）直接调用自定义 Maven 路径下的可执行文件运行测试：`/Users/weiyaozhou/IdeaProjects/apache-maven-3.8.8-bin/apache-maven-3.8.8/bin/mvn test -Dtest=LeadTimeDemandSimulatorTest,TruncatedNormalSamplerTest`。执行后 6 个单元测试全部成功编译并通过。
Q - 需求预测结果点击「手动触发重算」报错「任务异常终止: Data truncation #22001」插入 ai_forecast_result 失败 - 根因是 StockThresholdService 写入 model_version=\"two-stage-python-yyyy-MM\"（25 字符），而表列 model_version 为 varchar(20)；MySQL 在 STRICT 模式下对字符串超长返回 #22001。解决方案：1) 执行 sql/fix_ai_forecast_result_model_version_len.sql 将列扩为 varchar(64)；2) 同步更新 ai_module.sql / forecast_m8_complete_schema.sql 建表定义；3) 落库前对 predict_qty 等 DECIMAL 字段 setScale 并钳制 occurrence_prob 到 [0,1]，避免 DOUBLE 精度尾数/越界再次触发截断。本地库已 ALTER 验证 model_version 现为 varchar(64)。重启后端后重新点击手动重算即可。
Q - 手动触发重算修复 ai_forecast_result 后仍报 #22001，错误定位变为 AiModelRegistryMapper 插入 ai_model_registry - 根因相同字段过短：代码写入 model_version=\"two-stage-python-yyyy-MM\"（24 字符），而 ai_model_registry.model_version 仍为 varchar(20)；流水线先写预测结果再注册模型，故前表修好后错误后移到注册表。解决方案：ALTER model_version/model_name 为 varchar(64)，迁移脚本见 sql/fix_ai_model_registry_model_version_len.sql（已并入 fix_ai_forecast_result_model_version_len.sql）；本地库已扩列并通过完整 INSERT 冒烟。无需重启后端，再次点击手动重算即可。
Q - AI任务中心补货为何不走两阶段且 FAILURE - 原链路固定 Celery→smart_replenishment 导入 TensorFlow，与 NumPy 不兼容导致 TASK_FAILED；已改为 Java 任务中心直接调度两阶段 Hurdle-Gamma（StockThresholdService+ReplenishmentService），不再依赖 LSTM/TF。重启后端后提交 C0010003 等编码即可。
Q - 需求预测模块多处逻辑不一致如何收敛 - 已统一目标月为下月；关闭旧RF/SBA定时防覆盖；训练看板增加刷新与日批并标明离线日表≠Hurdle-Gamma月特征；任务中心状态写入ai_task_result；FALLBACK命名去歧义。重启后端后生效；看板点「刷新训练集」或等每日00:30。
Q - 训练数据看板全是0且业务逻辑不合理如何处理 - 根因是密面板+按最新日排序+业务停更尾巴；已改默认非插补/出库量排序/健康度meta/生成窗口贴齐业务日；并写入近6个月合理领用种子后重建日表，非零行约7%。重启后端并刷新看板即可。

Q - 侧栏仍见周粒度预测；库存分析 ABC/滞库无数据；维修费用分析“加载失败” - 排查结论与修复：
1) **周粒度侧栏**：库中 `menu` 已无 `/ai/weekly-forecast`、训练看板等项；`/api/menus/my` 仅返回「需求预测结果」「AI任务中心」。前端 `Home.vue` 增加 `isHiddenMenu`（`[隐藏]`、名称含周粒度/训练数据/训练进度、path 匹配 offline 路由），`MenuMapper.findMenusByUserId` 同步过滤。
2) **ABC 无数据**：后端 SQL/接口本身有数据（A12/B10/C28），根因是前端字段映射错误（用了 `abcClass/count`，接口实际为 `classLevel/partCount`）。已在 `InventoryReport.vue` 兼容映射并过滤 value>0。
3) **滞库无数据**：接口逻辑正确；因近期种子领用把多数备件“刷活跃”。已对 id 21–28 清理近 200 天领用明细并抬高库存/拉长 `created_at`，滞库清单现约 19 条（阈值 90 天）。
4) **维修费用加载失败**：前端曾请求不存在的 `/report/maintenance/cost` 与 `/device-top10`（404）。已改为 `/cost-by-month`、`/cost-by-device`；库内 `biz_work_order` 费用齐全，并补 2026-07 工单 15 条。Axios 超时由 5s 调整为 30s。
验证：代理 `localhost:3000` 下 ABC/滞库/费用接口均 200；AI 菜单仅 forecast-result + job-center。刷新页面或重新登录即可看到效果。

Q - 维护建议管理全是待处理、健康评分 0 分、预估成本全是 2000 元 - 根因三层：1) 库内状态写成 APPROVED，前后端标准为 ACCEPTED，导致「已采纳」统计为 0、状态展示异常；2) 列表接口字段为 currentHealthScore，前端读 healthScore，缺字段被 formatDecimal 显示为 0.00；3) 生成逻辑在无配套备件时只剩人工费（紧急维护固定 2000），且种子数据曾整批写相同成本。解决方案：执行 sql/fix_maintenance_suggestion_realistic.sql 重刷真实分布数据；前端 getHealthScore 兼容双字段名；实体 @JsonProperty 输出 healthScore/riskLevel/handledByName；列表 SQL 联查故障概率与处理人；MaintenanceSuggestionGenerator 无备件时按类型给默认备件费。刷新页面后可见多状态、非零健康分、差异化成本；后端改动需重启 Spring Boot 后 JSON 字段名与故障概率列完全生效。
