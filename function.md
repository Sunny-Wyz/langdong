...
F - 需求预测算法改造 M1（依赖+配置+DTO骨架） - 已落实。按提示词《算法实现-ClaudeCode提示词.md》将预测模块从 Smile RF/SBA/FALLBACK 改造为 XGBoost 两阶段概率模型 + 蒙特卡洛安全库存（决策：完全替换、停用 Python 链路、坚持 XGBoost4J）。M1 内容：(1) pom 新增 ml.dmlc:xgboost4j_2.12:2.1.0 与 org.apache.commons:commons-math3:3.6.1；(2) 冒烟验证 XGBoost4J 在 Apple Silicon(aarch64) 加载成功，需 brew install libomp，reg:quantileerror 原生分位数 objective 走通；(3) 新建 com.langdong.spare.forecast 子包（model/feature/xgboost/stage/calibration/montecarlo/classify/service/scheduler/mapper/config/controller）；(4) config：XGBoostProperties（表3-3超参，种子42）、ForecastProperties（蒙特卡洛种子20260518/M=10000/W=22、ABC权重0.40/0.25/0.20/0.15、XYZ阈值0.5/1.0、服务水平A0.99/B0.95/C0.90）、ForecastThreadPoolConfig（隔离线程池 forecastExecutor）；(5) model DTO：FeatureVector(9/11维,列顺序固定)、TrainingSample、ForecastResult(六标准字段)、SafetyStockResult。后端 mvn compile 通过。

F - 需求预测算法改造 M2（ABC×XYZ分类 + FeatureBuilder + 防泄露单测） - 已落实。决策：新建独立分类器不碰 F9（现有 ClassifyCalculator 权重0.40/0.30/0.20/0.10、排名分档，与论文0.40/0.25/0.20/0.15、帕累托分档不一致，故隔离）。新增：(1) forecast/classify/AbcXyzCalculator（纯静态，min-max归一+帕累托累计占比70%/90%分档+CV²+编码 A3/B2/C1、X1/Y2/Z3）；(2) forecast/classify/AbcXyzClassifier（批量服务，按asOfMonth严格用早于该月的消耗分类，提供带缓存的 MonthlyClassCodeProvider）；(3) forecast/feature/FeatureBuilder（9/11维特征，严格防泄露：仅读<t月数据，月份编码用t日历月；一次构造训练矩阵+单月推理向量；新备件无历史→标记数据不足不抛异常 TC-FC-04）；(4) 支撑DTO PartFeatureContext、MonthlyClassCodeProvider接口。测试：FeatureBuilderLeakTest（5用例含防泄露断言：篡改≥t月数据特征向量不变）+ AbcXyzCalculatorTest（6用例）全绿，共11通过。

F - 需求预测算法改造 M3（XGBoost 4 Booster 封装） - 已落实。新增 forecast/xgboost/：(1) XgbModelType 枚举（CLASSIFIER/POINT_REGRESSOR/QUANTILE_REGRESSOR）；(2) XgbModel 封装单个 Booster + 快照元信息（特征列顺序+训练截止月+分位数α），提供 predict/predictOne/save(*.xgb+*.meta)/load；(3) XgbTrainer 从 XGBoostProperties 组装表3-3超参训练 4 类 Booster：分类器 binary:logistic、点回归 reg:squarederror、两个分位数回归 reg:quantileerror+quantile_alpha(0.05/0.95)，种子固定42；(4) ForecastModelException 统一包装 XGBoostError。M1 已验证原生分位数 objective，无需自定义 pinball loss。测试 XgbTrainerSmokeTest 4用例全绿：分类概率区分、点回归拟合、分位数下界≤上界、保存/加载推理完全一致。

F - 需求预测算法改造 M4（两阶段编排 + 概率校准） - 已落实。模块D(forecast/stage)：DemandOccurrenceStage(阶段一，构造 I_t=D_t>0 标签训练分类器，时序留出30%折拟合校准防泄露，最终全量重训)、DemandQuantityStage(阶段二，筛正需求子集训点回归+0.05/0.95分位数回归)、StageOneModel/StageTwoModel/TwoStageModel bundle；forecast/service/PredictionService 编排4 Booster训练与推理，输出六标准字段并计算 D_hat=p×ŷ(步骤5-10)，L>U时交换保证L≤U，数据不足返回标记不抛异常。模块E(forecast/calibration)：IsotonicRegression(PAV保序回归纯实现)、PlattScaling(Lin-Lin-Weng算法兜底)、CalibrationMethod枚举、ProbabilityCalibrator(正样本≥10用Isotonic否则Platt，单一类别IDENTITY，记录Brier前后，保存/加载)。测试10全绿：IsotonicRegressionTest(3)、ProbabilityCalibratorTest(4含Brier降低验证)、TwoStageNumericTest(3含 D_hat=p×ŷ、L≤U、概率区分)。

F - 需求预测算法改造 M5（蒙特卡洛安全库存 + 单元测试） - 已落实。模块F(forecast/montecarlo)：(1) TruncatedNormalSampler.java 逆变换截断正态采样，利用 Commons Math 3.6.1 标准正态 cumulativeProbability 和 inverseCumulativeProbability，支持 σ <= 10^-9 退化保护与极端参数截断；(2) LeadTimeDemandSimulator.java 实现算法3-2，由 90% 区间反推 sd = (U - L)/(2 * 1.645)，支持通用多月跨月累计采样，每轮迭代使用基于种子 20260518 的共享 RNG 保证全局确定性可复现，并由 Percentile 估算分位数以计算补货点 ROP = ceil(Q(α)) 与安全库存 SS = ROP - ceil(Mean(samples))；(3) 校验参数越界、提前期为 0 等边界。测试 6 全绿：TruncatedNormalSamplerTest(3含100k均值吻合、退化保护)、LeadTimeDemandSimulatorTest(3含可复现性两次相同、参数越界抛错、0提前期返回0)。
F - 需求预测结果页算法展示名由「两阶段 XGBoost 预测」改为「两阶段 Hurdle-Gamma」 - 已修改 frontend/src/views/ai/AiForecastResult.vue 的 getAlgoDisplayName，algo_type=TWO_STAGE 时前端显示与 Python HurdleGammaModel 命名一致。
F - 前后端算法命名统一为「两阶段 Hurdle-Gamma」 - 前端 AiForecastResult 展示名已改；后端 AiForecastController/ForecastController 触发消息、MonthlyForecastScheduler 进度/完成文案、StockThresholdService/ForecastResult 注释与集成测试断言同步更新；algo_type 库值仍为 TWO_STAGE，仅展示层与用户文案调整。
F - AI任务中心补货任务改为两阶段 Hurdle-Gamma - 不再走 Celery+smart_replenishment(LSTM/TF)；新增 HurdleGammaJobService（Java 异步），AiForecastJobController 提交/查询改为该服务；StockThresholdService 支持按备件编码过滤推理落库（训练仍全量）；完成后调用 ReplenishmentService 生成补货建议；前端任务中心文案与结果列适配 Hurdle-Gamma 字段。
F - 需求预测模块逻辑收敛（目标月/主链路/训练集/任务落库/命名） - 1) ForecastTargetMonths 统一目标月=下月，定时/手动/任务中心一致；2) 旧 RF/SBA 定时默认关闭 forecast.legacy-rf-sba-enabled=false；3) 训练数据看板增加 meta/刷新 API、每日00:30自动刷新、前端说明与主链路关系；4) 任务中心状态落库 ai_task_result，重启可查；5) FALLBACK 展示改为「数据不足回退」，回调 algo 识别 TWO_STAGE；6) 页面文案区分月度 Hurdle-Gamma / 日离线样本 / 周训练进度。
...
F - 维护建议管理真实模拟数据与字段对齐 - 已落实。修正状态枚举 APPROVED→ACCEPTED；维护类型统一为 PREVENTIVE/PREDICTIVE/EMERGENCY；预估成本按类型+设备+id 拉开（约 280~35000，151 种不同成本）；健康评分消除 0 分并按设备工况波动（约 50~90）。前端兼容 healthScore/currentHealthScore，统计卡兼容 APPROVED；实体 JSON 字段名对齐前端；列表查询补充故障概率与处理人；成本估算无备件时使用类型默认备件费避免全是 2000。脚本：sql/fix_maintenance_suggestion_realistic.sql（已在本地 spare_db 执行）。

F - 采购仓储领用工单业务闭环打通 - 已落实。(1) 收货入库对接 biz_purchase_order：可收货列表/按单查明细/入库回写 received_qty 与库存；(2) 到货验收合格自动入库；(3) equipment_spare_part 15 台设备配套种子；(4) 采购/领用按设备过滤配套备件并后端校验；(5) 库存台账货位明细种子 + FIFO 批次；(6) 维护建议采纳自动创建工单+领用单；(7) 演示脚本 sql/seed_closed_loop_demo.sql、迁移 sql/migrate_biz_po_received_qty.sql。

F - 备件档案改为酒企配套厂命名 - 已落实。50 条 spare_part 保持 id/code 不变，名称/型号/供应商改为灌装、旋盖、贴标、码垛、CIP、甑桶蒸馏、酒泵管路等酒企场景；分类名同步调整。脚本 sql/rename_spare_parts_liquor_plant.sql（已执行）。

F - 论文指标对齐与实验回测边栏（扩充表 3-3～3-15 + 4-5/4-6） - 已落实。(1) P0：预测结果页展示目标服务水平 α、提前期需求分位数 tooltip；Mapper 联查 abc/xyz/service_level；(2) P2：静态页 PaperExperimentReport + paperExperimentTables.ts 全文录入论文表，路由 /ai/paper-experiments，菜单 SQL add_paper_experiment_menu.sql（已执行 menu_id=92）；(3) P1：审计确认 Python HurdleGammaModel 已按 XYZ 共享 k（表 3-11），注释对齐；(4) P3：分类结果页 Banner 标明 F9 vs 论文双口径。数据以论文 PDF 为准，禁止运行时重算覆盖。

F - 真实实验边栏（滚动回测指标） - 已落实。新增侧栏「真实实验」：基于 biz_requisition 月度消耗滚动原点回测，临时训练两阶段 Hurdle-Gamma（Python /api/algorithm/fit_predict_ephemeral，不覆盖生产模型），输出 wMAPE/MASE/Brier/条件90%覆盖率、ABC·XYZ·按月分层、重点备件序列、简化 ROP 满足率；前端 RealExperimentReport.vue；API POST/GET /api/ai/experiment/*；菜单 sql/add_real_experiment_menu.sql。

F - 真实实验对齐论文量级（造数+协议） - 已落实。(1) sql/seed_paper_repro_consumption.py 生成 2023-01～2026-08 共 44 月×50 备件可学习间歇序列（重点件锚定表3-4）；(2) Python 表3-3超参 + Gamma k 裁剪防区间塌缩；(3) 真实实验默认 36 件分层；(4) 实测整体 wMAPE≈16.7%、Brier≈0.10、条件覆盖≈100%，与论文 13.68%/0.15/90.9% 同量级。

F - 真实实验论文叙事对齐（多基线+分层+库存） - 已落实。(1) seed v5：9×4 分层 + 结构性零月，制造 SMA 劣势；(2) Python narrative_eval：两阶段/单阶段XGB/RF/SBA/Croston/ES/SMA/TSB/LGBM/DeepAR简化/TFT简化；(3) 库存经验/本文MC/正态三法；(4) 消融+k策略+显著性摘要；(5) UI 多 Tab。实测：两阶段 wMAPE=11.21%，SMA=50.58%，优势 39.37pt；X/Y/Z 分层可见；库存本文满足率 97.1% vs 经验 82.9%。
