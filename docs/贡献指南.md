...
| 命令 | 说明 |
|------|------|
| `npm run dev` | Vite 开发服务器（热重载），地址：http://localhost:3000 |
| `npm run build` | `vue-tsc` + 生产构建，输出至 `dist/` |
...
- 使用 Element Plus，不要引入额外 UI 库
- 路由守卫在 `src/router/index.ts`；新增路由需匹配后端菜单权限
- API 请求统一通过 `src/utils/request.ts` 的 axios 实例（自动带 Bearer，401 时刷新 token）
