...
本项目生产主链路为 **两阶段 Hurdle-Gamma + 蒙特卡洛提前期仿真**（Java 编排，Python 训练/推理/仿真）。算法推导见 [两阶段Hurdle-Gamma与蒙特卡洛](两阶段Hurdle-Gamma与蒙特卡洛.md)。本文记录可配置参数与回测入口。

> 训练走 `POST /api/algorithm/train`，不再在 JVM 内用 XGBoost4J 落本地快照做全量拟合。
...
- **随机数种子 (RNG Seed)**：Java 配置为 **`20260518`** (`forecast.monteCarlo.seed = 20260518`)。**注意：当前 Python `simulate_lead_time_demand()` 未接收该种子**，两次全量重算的 ROP/SS 可能差 1 件量级。
