...
流程：
1. 按目标月（默认下月）从领用出库汇总历史月消耗
2. Java `FeatureBuilder` 构造 36 个月训练样本与当月推理向量
3. Python 两阶段 Hurdle-Gamma 训练 / 推理，得到 \(p_t,\mu_t\) 与 90% 区间
4. 蒙特卡洛仿真提前期需求，计算 ROP / SS
5. 写入 `ai_forecast_result` 与 `biz_part_classify`，再生成补货建议
...
输出：
- 无条件点预测 \(\hat D=p_t\mu_t\)
- 正需求 90% 区间、发生概率、正需求均值
- 安全库存 SS、补货点 ROP
...
- 架构全景: [系统总体架构](系统总体架构.md)
- FIFO 实现细节: [../IMPLEMENTATION/FIFO出库指南.md](../IMPLEMENTATION/FIFO出库指南.md)
- AI 预测与补货: [../AI_ALGORITHMS/两阶段Hurdle-Gamma与蒙特卡洛.md](../AI_ALGORITHMS/两阶段Hurdle-Gamma与蒙特卡洛.md)
- 数据库字段定义: [../DATABASE/数据模型.md](../DATABASE/数据模型.md)
