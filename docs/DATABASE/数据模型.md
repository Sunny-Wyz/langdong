...
**数据库**: `spare_db`（utf8mb4）  
**最后更新**: 2026-08-13  
**口径**: 字段以本文件 + `sql/` 建表脚本为准。旧的 20 表速查草稿已并入本文。

查询模板在文末。完整建表语句仍以 `sql/init.sql` 及各 `*_module.sql` 为准。
...
| occurrence_prob | DECIMAL(5,4) | 需求发生概率 p_t |
| positive_qty | DECIMAL(8,2) | 正需求条件均值 μ_t |
| lead_time_quantile | DECIMAL(8,2) | 提前期需求分位数（诊断） |
| algo_type | VARCHAR(20) | 预测算法：TWO_STAGE / FALLBACK |
...
SELECT
    ri.spare_part_id,
    DATE_FORMAT(COALESCE(r.approve_time, r.apply_time), '%Y-%m') AS month,
    SUM(ri.out_qty) AS month_consume
FROM biz_requisition_item ri
JOIN biz_requisition r ON ri.req_id = r.id
WHERE r.req_status IN ('OUTBOUND', 'INSTALLED')
  AND ri.out_qty > 0
  AND DATE_FORMAT(COALESCE(r.approve_time, r.apply_time), '%Y-%m') = '2026-02'
...
JOIN biz_part_classify bpc ON sp.code = bpc.part_code
